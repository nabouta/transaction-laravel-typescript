...
import {
     nomComplet, port, fournisseurSelect,numeroDes,nomCompletDes,notification,
    transactionSelect, montant, clientTelephone, clientname, submitClient,
    fournisseurSelect2, numeroAjoutCompte, submitCompte, listCompte,colfour,
    rechercherCompte, modal, afficheFournisseur, afficheName, afficheSolde, afficheTelephone,
    close, block, deblock, deleteCompte, afficheStatut, historique, modalHistorique, closeHistorique, expediteurNumero, tbody,submitTransaction,error,codeT, expediteurNom,modalCompte
} from "./dom.js";
...
function showCompte(compte: any, modal: any) {
...
rechercherCompte.addEventListener("input", async () => {
    ...
    try {
        ...
        if (Array.isArray(comptes)) {
            const matchingComptes = comptes.filter((compte: any) =>
                compte.client.nomComplet.toLowerCase().includes(searchTerm) || compte.client.telephone.includes(searchTerm)
            );
            matchingComptes.forEach((compte: any) => {
    ...
    try {
        ...
        if (Array.isArray(responses)) {
            ...
            responses.forEach(Transaction => {
                const li = createHistorique(Transaction.id, Transaction.montant, Transaction.type_transfert
